...
export class SophosPartnerConnector {
  ...
  constructor(config: SophosPartnerConfig) {
    this.client = new SophosHTTPClient(config);
    ...
    return {
      tenants: {
        async list(params?) {
          ...
          const tenants = await client.fetchAllPages<SophosPartnerTenant>(url, headers);
  ...
  private buildFirewallNamespace(): SophosPartnerConnector['firewall'] {
    ...
    const firewallMethods = {
      async list(tenantConfig: SophosTenantConfig, params?: { pageSize?: number }) {
        const headers = await client.tenantHeaders(tenantConfig.tenantId);
        const url = `${tenantConfig.apiHost}/firewall/v1/firewalls?pageTotal=true&pageSize=${params?.pageSize ?? 100}`;
        const firewalls = await client.fetchAllPages<SophosPartnerFirewall>(url, headers);
    ...
    return {
      ...
      firewalls: {
        async list(scope) {
          ...
          const result = await client.get<{ items: SophosPartnerFirewallLicense[] }>(
