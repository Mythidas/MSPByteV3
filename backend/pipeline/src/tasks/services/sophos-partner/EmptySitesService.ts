...
/**
 * Identifies Sophos-managed company/site entities with no endpoint children.
 * A site is "empty" if it has no endpoint entities assigned to it (site_id matches).
 */
export class SophosEmptySitesService {
  ...
  async analyze(tenantId: string, scope: ScopeDefinition): Promise<EntityRow[]> {
    ...
    if (companyError) {
      throw new Error(`SophosEmptySitesService: failed to load companies — ${companyError.message}`);
    }
    ...
    if (endpointError) {
      throw new Error(
        `SophosEmptySitesService: failed to load endpoints — ${endpointError.message}`
      );
    }
    ...
    Logger.trace({
      module: 'SophosEmptySitesService',
      context: 'analyze',
      message: `Found ${empty.length} empty Sophos sites for tenant ${tenantId}`,
    });
...
export function getSophosEmptySitesService() {
  return new SophosEmptySitesService(getSupabase());
}
