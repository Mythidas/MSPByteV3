...
/**
 * Identifies Sophos-managed endpoints that have been offline beyond a threshold.
 * Uses raw_data.lastSeenAt and raw_data.online to determine offline status.
 */
export class SophosOfflineDevicesService {
  ...
  async analyze(
    ...
    if (error) {
      throw new Error(`SophosOfflineDevicesService: failed to load endpoints — ${error.message}`);
    }
    ...
    Logger.trace({
      module: 'SophosOfflineDevicesService',
      context: 'analyze',
      message: `Found ${offline.length} offline Sophos endpoints for tenant ${tenantId} (threshold: ${thresholdDays}d)`,
    });
...
export function getSophosOfflineDevicesService() {
  return new SophosOfflineDevicesService(getSupabase());
}
