...
export async function writeResults(
  ...
  const rows = results.map((r) => ({
    ...
    status: r.status,
    detail: isJson(r.detail) ? r.detail : {},
    evaluated_at: now,
  }));
  ...
  for (const r of results) {
    ...
    if (r.onChangeWorkflowId && prev !== null && prev !== curr) {
      Logger.info({
        ...
        meta: {
          workflowId: r.onChangeWorkflowId,
          frameworkCheckId: r.frameworkCheckId,
          linkId: r.linkId,
          prev,
          curr,
        },
