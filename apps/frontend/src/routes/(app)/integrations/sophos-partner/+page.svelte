<script lang="ts">
  ...
  const form = superForm(data.form, {
    validators: zod4Client(sophosConfigSchema),
    resetForm: false,
    onUpdate({ form }) {
      if (form.message) {
        if (form.valid) {
  ...
  <div class="flex bg-card w-1/2 h-fit py-2 px-4 shadow rounded">
    <Breadcrumb.Root>
      <Breadcrumb.List>
        ...
        <Breadcrumb.Item>
          <Breadcrumb.Page>Sophos Partner</Breadcrumb.Page>
        </Breadcrumb.Item>
      </Breadcrumb.List>
    </Breadcrumb.Root>
  </div>
      ...
      >
        <Tabs.Content value="configuration" class="w-full h-full overflow-hidden space-y-4">
          <div class="flex items-center justify-between">
            <div>
              <h2 class="text-lg font-semibold">Sophos Partner Configuration</h2>
              <p class="text-sm text-muted-foreground">
                Configure your Sophos Partner integration credentials
              </p>
            </div>
            {#if data.integration}
              <Button
        ...
        <Tabs.Content value="mappings" class="w-full h-full overflow-hidden">
          {#if data.integration}
            ...
            {:then mappingData} 
              <Mapper id="sophos-partner" sites={data.sites} tenants={mappingData.tenants} links={mappingData.links} />
            {:catch error}
