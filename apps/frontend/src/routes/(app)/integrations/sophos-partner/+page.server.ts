// src/routes/(app)/integrations/sophos-partner/+page.server.ts
...
const fetchTenants = async (orm: ORM) => {
  ...
  const sophos = new SophosPartnerConnector(data.config as any);
  const { data: companies, error } = await sophos.getTenants();
...
export const load: PageServerLoad = async ({ locals }) => {
  ...
  const form = await superValidate(formDefaults, zod4(sophosConfigSchema));
...
export const actions: Actions = {
  testConnection: async ({ request, locals }) => {
    const form = await superValidate(request, zod4(sophosConfigSchema));
    ...
    try {
      ...
      const connector = new SophosPartnerConnector(configData as any);
  ...
  save: async ({ request, locals }) => {
    ...
    try {
      ...
      const connector = new SophosPartnerConnector(configData);
  ...
  delete: async ({ locals }) => {
    try {
      ...
      const { error } = await locals.orm.delete('public', 'integrations', (q) =>
        q.eq('id', 'sophos-partner').eq('tenant_id', locals.user?.tenant_id || '')
      );
