  ...
  <div class="flex bg-card w-1/2 h-fit py-2 px-4 shadow rounded">
    <Breadcrumb.Root>
      <Breadcrumb.List>
        ...
        <Breadcrumb.Item>
          <Breadcrumb.Page>Cove</Breadcrumb.Page>
        </Breadcrumb.Item>
      </Breadcrumb.List>
    </Breadcrumb.Root>
  </div>
      ...
      >
        <Tabs.Content value="configuration" class="w-full h-full overflow-hidden space-y-4">
          <div class="flex items-center justify-between">
            <div>
              <h2 class="text-lg font-semibold">Cove Configuration</h2>
              <p class="text-sm text-muted-foreground">
                Configure your Cove integration credentials
              </p>
            </div>
            {#if data.integration}
              <Button
            ...
            <div class="space-y-4">
              ...
              <div class="space-y-2">
                ...
                <Input
                  ...
                  type="number"
                  placeholder="777777"
                  bind:value={$formData.partnerId}
                  aria-invalid={$errors.partnerId ? "true" : undefined}
                  class={$errors.partnerId ? "border-destructive" : ""}
                />
                {#if $errors.partnerId}
                  <p class="text-sm text-destructive flex items-center gap-1">
                    <AlertCircle class="h-3 w-3" />
        ...
        <Tabs.Content value="mappings" class="w-full h-full overflow-hidden">
          {#if data.integration}
            ...
            {:then mappingData} 
              <Mapper id="cove" sites={data.sites} tenants={mappingData.tenants} links={mappingData.links} />
            {:catch error}
