  ...
  const healthConfig = $derived.by(() => [
    {
      ...
      class: data.identityHealth.mfaRisk > 0 ? 'text-destructive' : 'text-muted-foreground',
    },
    {
      label: 'Stale Users',
      count: data.identityHealth.staleUsers,
      class: data.identityHealth.staleUsers > 0 ? 'text-yellow-500' : 'text-muted-foreground',
    },
    {
      label: 'License Waste',
