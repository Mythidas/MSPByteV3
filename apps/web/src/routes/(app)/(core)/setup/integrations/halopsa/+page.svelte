<script lang="ts">
  ...
  $effect(() => {
    data.getHalopSites.then((result) => {
      externalOptions = (result ?? []).map((h) => ({
        id: String(h.id),
        name: h.clientsite_name,
        meta: { siteName: h.clientsite_name },
      }));
      ...
      <Sheet.Content side="right" class="w-105 flex flex-col gap-0 p-0">
        ...
        >
          ...
          <Sheet.Footer class="flex justify-between p-4 border-t gap-2">
            {#if !!dbIntegration}
              <ConfirmDialog
                title="Delete HaloPSA Integration?"
                description="This will remove the HaloPSA integration and all associated site mappings. This action can be undone within 30 days."
