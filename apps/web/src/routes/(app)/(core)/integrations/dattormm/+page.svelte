<script lang="ts">
  ...
  let dattoSites = $state<DattoRMMSite[]>([]);
  let loading = $state(true);
  let loadingDattoSites = $state(false);
  ...
  $effect(() => {
    if (dbIntegration) loadDattoSites();
  });
  ...
  async function loadDattoSites() {
    loadingDattoSites = true;
    try {
      const res = await fetch('/api/integrations/dattormm/sites');
      if (!res.ok) {
        const body = await res.json().catch(() => ({}));
        throw new Error(body.error ?? res.statusText);
      }
      dattoSites = await res.json();
    } catch (err) {
      toast.error(`Failed to load DattoRMM sites: ${err}`);
    } finally {
      loadingDattoSites = false;
    }
  }
  ...
  function runAutoMap() {
    ...
    for (const site of dbSites) {
      ...
      for (const dattoSite of dattoSites) {
        if (taken.has(dattoSite.uid)) continue;
        const score = jaccardSimilarity(site.name, dattoSite.name);
        if (score > bestScore) {
          bestScore = score;
          bestUid = dattoSite.uid;
        }
      }

      if (bestUid) {
        next[site.id] = bestUid;
        taken.add(bestUid);
      }
    }
  ...
  async function handleSaveAll() {
    ...
    try {
      ...
      for (const site of dbSites) {
        ...
        if (pending) {
          const dattoSite = dattoSites.find((d) => d.uid === pending);
          toUpsert.push({
            integration_id: 'dattormm',
            tenant_id: tenantId,
            site_id: site.id,
            external_id: pending,
            name: dattoSite?.name ?? pending,
            status: 'active',
            meta: dattoSite ? { siteName: dattoSite.name } : {},
          });
        } else if (committed) {
          const link = dbLinks.find((l) => l.site_id === site.id);
      ...
      <div class="flex gap-2 ml-auto shrink-0">
        <PermissionGaurd permission="Integrations.Write">
          <Button
            ...
            size="sm"
            class="gap-2"
            disabled={loadingDattoSites || dattoSites.length === 0}
            onclick={runAutoMap}
          >
            <Wand2 class="size-4" />
            AutoMap
        ...
        <div class="flex-1 overflow-y-auto">
          <FadeIn class="flex flex-col divide-y">
            {#each filteredSites as site (site.id)}
              ...
              {@const rowOptions = dattoSites
                .filter((d) => !takenByOthers.has(d.uid))
                .map((d) => ({ label: d.name, value: d.uid }))}
