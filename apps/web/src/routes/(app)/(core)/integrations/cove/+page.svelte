<script lang="ts">
  ...
  let customers = $state<CoveChildPartner[]>([]);
  ...
  $effect(() => {
    if (dbIntegration) loadCustomers();
  });
  ...
  async function loadCustomers() {
    ...
    try {
      const res = await fetch('/api/integrations/cove/customers');
      if (!res.ok) {
      ...
      toast.error(`Failed to load Cove customers: ${err}`);
    } finally {
  ...
  function runAutoMap() {
    ...
    for (const site of dbSites) {
      ...
      for (const customer of customers) {
        const customerId = String(customer.Info.Id);
        if (taken.has(customerId)) continue;
        const score = jaccardSimilarity(site.name, customer.Info.Name);
        if (score > bestScore) {
          bestScore = score;
          bestId = customerId;
        }
  ...
  async function handleSaveAll() {
    ...
    try {
      ...
      for (const site of dbSites) {
        ...
        if (pending) {
          const customer = customers.find((c) => String(c.Info.Id) === pending);
          toUpsert.push({
            integration_id: 'cove',
            tenant_id: tenantId,
            site_id: site.id,
            external_id: pending,
            name: customer?.Info.Name ?? pending,
            status: 'active',
            meta: customer ? { customerName: customer.Info.Name } : {},
          });
        } else if (committed) {
          const link = dbLinks.find((l) => l.site_id === site.id);
              ...
              <AlertDialog.Content>
                <AlertDialog.Header>
                  ...
                  <AlertDialog.Description>
                    This will remove the Cove Backups integration and all associated site mappings.
        ...
        <div class="flex-1 overflow-y-auto">
          <FadeIn class="flex flex-col divide-y">
            {#each filteredSites as site (site.id)}
              ...
              {@const rowOptions = customers
                .filter((c) => !takenByOthers.has(String(c.Info.Id)))
                .map((c) => ({ label: c.Info.Name, value: String(c.Info.Id) }))}
              <div class="grid grid-cols-[1fr_1fr] gap-4 px-4 py-3 items-center hover:bg-muted/30">
                ...
                <SingleSelect
                  ...
                  placeholder={loadingCustomers ? 'Loading...' : 'Select a Cove customer...'}
